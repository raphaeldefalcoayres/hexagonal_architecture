  ...
  implements ParkedCarRepository
{
  ...
  async save(parkedCar: ParkedCar): Promise<void> {
    await this.connection.query(
      "insert into parked_car (plate, checkin_date) values  ($1, $2)",
  ...
  async update(parkedCar: ParkedCar): Promise<void> {
    await this.connection.query(
      "update parked_car set checkout_date = now() where plate = $1",
      [parkedCar.plate]
    );
  }
  ...
  async list(): Promise<ParkedCar[]> {
    const parkedCarsData = await this.connection.query(
      "select * from parked_car where checkout_date is null",
      []
    );
  ...
  async get(plate: string): Promise<ParkedCar> {
    const parkedCarData = await this.connection.one(
      "select * from parked_car where plate = $1 and checkout_date is null",
      [plate]
    );
    const parkedCar = new ParkedCar(
      parkedCarData.plate,
